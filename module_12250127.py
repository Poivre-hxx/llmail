...
class Movement:
    ...
    def update(self, game_object, path):
        ...
        target_pos = path[1]  # Simplified path following
...
class Attack:
    ...
    def update(self, game_object, monsters):
...
class GameManager:
    def __init__(self):
        ...
        self.towers = []
        self.monsters = []
        self.path = [(0, SCREEN_HEIGHT // 2), (SCREEN_WIDTH, SCREEN_HEIGHT // 2)]
        self.wave_number = 0
    ...
    def update(self, dt):
        ...
        for monster in self.monsters:
            ...
            if monster.position[0] > SCREEN_WIDTH:
                self.monsters.remove(monster)
                self.monsters_escaped += 1

        # Update towers
        for tower in self.towers:
            if tower.components["attack"].update(tower, self.monsters):
    ...
    def spawn_wave(self):
        ...
        for _ in range(5 + self.wave_number):
            monster = Monster(f"monster_{len(self.monsters)}", [0, SCREEN_HEIGHT // 2], 100, 1)
            self.monsters.append(monster)

    def place_tower(self, position):
        if self.currency >= 50:
            tower = Tower(f"tower_{len(self.towers)}", position, 30, 100)
...
def main():
    ...
    while running:
        ...
        # Draw path
        pygame.draw.line(screen, GREEN, game_manager.path[0], game_manager.path[1], 5)
